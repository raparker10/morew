...
CFileLengthToSizeHelper FileLengthSizeHelper;
CMoreScreenLayout ScreenLayout(&FileLengthSizeHelper);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		handle_resize(hWnd, GetDC(hWnd), rc.right - rc.left, rc.bottom - rc.top);
		break;
...
void handle_resize(HWND hWnd, HDC hdc, int iWidth, int iHeight)
{
	...
	FileLengthSizeHelper.put_TextMetrics(&DrawInfo.tm);

	int iDefaultWidth = ScreenLayout.get_DefaultCharsPerLine(iWidth);
...
bool get_position(int x, int y, Section *section, int *line, int *column)
{
	...
	*section = Section::UNKNOWN;
	...
	else if (PtInRect(ScreenLayout.get_FilePosPanel(), p))
	{
		...
		*line = (y - ScreenLayout.get_FilePosPanel()->top) / DrawInfo.tm.tmHeight;
		*column = 0;
	}
